...
import type EditTaskProps from "../../types/EditTaskProps"

export default function EditToDoForm({ onSubmit, onClose, task }: EditTaskProps) {
	...
	const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
		...
		const formData: Task = {
			id, // Generate a temporary id for the new task
			title,
			description,
			status,
			priority,
	...
	return (
		<div className="bg-white rounded-[8px] px-[20px] py-[24px] max-w-[494px] w-full absolute transform-gpu top-1/2 and left-1/2 -translate-x-1/2 and -translate-y-1/2 z-10">
			<div className="font-bold text-[20px]">Edit task</div>
